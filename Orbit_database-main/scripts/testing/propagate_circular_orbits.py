...
if moon_plot:
    #x0_values = [1.132]
    #x0_values = np.linspace(1-mu + 0.01, 1-mu + 0.135, 10)
    #x0_values = np.linspace(1-mu + 0.01, 1.05, 10)
    #x0_values = np.linspace(9.9674505304367522E-1, 1.0461439989565655, 10)
    x0_values = np.linspace(1.08, 1.131, 20)
else:
    #x0_values = np.linspace(-mu + 0.6, -mu + 0.8, 10)  # for example, 10 points  #0.28 - 0.3: no solution, > 0.31: above wanted energy, 
    x0_values = np.linspace(-mu + 0.3, -mu + 0.4, 2)
...
for x0 in x0_values:
    ...
    if x0 < 0.8:
        ...
        #vy0 = 0.09564674
        print("Previous vy0 guess: ", np.sqrt(circ_mu / r) - (x0 - (1 - 2*mu))) # + 1 - mu - (x0 + mu) = - (x0 - (1 - 2*mu))
    print("vy0 guess: ", vy0)

    T_2body =  2.0 * np.pi * np.sqrt(abs(x0 + mu)**3 / circ_mu) 
    print("Half-period guess:", T_2body/2)
    
    T_refined = refine_half_period_guess(x0, vy0, 1/4*T_2body)
...
if moon_plot:
    plt.title("Circular Orbits around the moon in the CR3BP Frame")
else:
    plt.title("Naive Circular Orbits around Earth in the CR3BP Frame")
plt.xlabel("x")
plt.ylabel("y")
plt.legend()
plt.axis("equal")
